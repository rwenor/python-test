...
def AxHex(hex):
    assert(hex[0]=='#')    
    return int(hex[1:], 16)*1000.0/0xffff
...
class VE:
    ...
    def add_pz(self, time, sens, sig):
        ...
        self.pz[sens].append((time - self.first, time - self.last[sens], sig))
        self.last[sens] = time
...
class VE_Pars:
    ...
    def __init__(self, skip=3):  # Skip
        self.ve = {}
        self.skip = skip
    
    def ParsLine(self, str):
        print str
        str = str[self.skip:] # skip length
        d = str.strip().split(',')
...
if __name__ == '__main__':
    ...
    class TestStringMethods(unittest.TestCase):
      ...
      def Xtest_ParsVE(self):
          print '\nLeser et kjt.'
          vep = VE_Pars()
          vep.ParsLine(str)

    
      def Xtest_ParsVE_lines(self):
          print '\nLeser et helt kjt.'
          vep = VE_Pars()
          for line in ve_list:
              vep.ParsLine(line)
              
      def test_ParsVE_file(self):
          print '\nLeser et helt kjt. fra fil'
          vep = VE_Pars(3)
          
          with open('interesan.axdmp', 'r') as f:
              for line in f:
                  vep.ParsLine(line)
    ...
    print '*'*40      
    print 'Test: '
    print "Tid?", (AxHex('#569673702509') -  AxHex('#5606E28073D1')) /(1000*3600*24)   
